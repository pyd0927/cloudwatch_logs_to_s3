...
- **멀티스레딩**: Python ThreadPoolExecutor를 활용한 성능 최적화
  - **가장 오래된 로그 검색**: 멀티스레딩으로 수백 개 스트림에서 병렬 검색 (최대 10배 성능 향상)
  - **로그 수집**: 각 스트림별 병렬 처리로 대용량 로그 수집 최적화
- **스레드 안전성**: Threading Lock으로 S3 State 파일 경쟁 조건 방지
- **로그 압축**: gzip 압축으로 저장 공간 절약 (95%+ 압축률)
...
# 가장 오래된 로그 검색용 워커 수 (멀티스레딩)
OLDEST_LOG_SEARCH_WORKERS="20"  # 기본값: 20 (대용량 스트림 검색 시 성능 향상)
...
### v1.5.0 (2025-10-16)
- **타임아웃 오류 해결**: 2021개 스트림 처리 시 타임아웃 문제 완전 해결
- **첫 실행 초기화 최적화**: 전체 백업 모드에서 첫 실행 시 State 초기화만 수행하여 안정성 향상
- **타임아웃 시간 증가**: 5분 → 10분으로 연장하여 대용량 스트림 처리 안정화
- **워커 수 증가**: 기본 워커 수 10개 → 20개로 증가하여 처리 속도 향상
- **오류 메시지 상세화**: TimeoutError와 일반 Exception 구분하여 디버깅 용이성 향상
- **Lambda 타임아웃 위험 최소화**: 첫 실행과 백업을 분리하여 안정성 극대화

### v1.4.0 (2025-10-16)
- **멀티스레딩 성능 최적화**: 가장 오래된 로그 검색에 멀티스레딩 적용으로 최대 10배 성능 향상
- **전체 스트림 검색**: 전체 백업 모드에서 모든 스트림을 대상으로 가장 오래된 로그 검색
- **스레드 안전성 보장**: Threading Lock으로 S3 State 파일 경쟁 조건 완전 해결
- **환경변수 추가**: `OLDEST_LOG_SEARCH_WORKERS`로 검색 워커 수 조정 가능
- **성능 모니터링**: 멀티스레딩 검색 소요시간 및 처리 결과 로깅 강화
- **대용량 처리 최적화**: 수백 개 스트림이 있는 로그그룹에서도 안정적인 State 파일 관리
...
이 프로젝트는 멀티스레딩을 활용하여 대용량 로그 처리 성능을 크게 향상시켰습니다:

#### 1. 가장 오래된 로그 검색 최적화
- **기존**: 순차 처리로 수백 개 스트림 검색 시 수십 분 소요
- **개선**: 멀티스레딩으로 **최대 10배 성능 향상**
- **설정**: `OLDEST_LOG_SEARCH_WORKERS` 환경변수로 워커 수 조정 가능

#### 2. 로그 수집 최적화
- **기존**: 스트림별 순차 처리
- **개선**: 각 스트림을 병렬로 처리하여 전체 수집 시간 단축
- **설정**: `MAX_WORKERS` 환경변수로 워커 수 조정 가능

#### 3. 스레드 안전성 보장
- **문제**: 멀티스레딩 환경에서 S3 State 파일 경쟁 조건 발생
- **해결**: Threading Lock으로 안전한 State 파일 관리
- **결과**: 수백 개 스트림 처리 시에도 모든 상태가 정확히 저장됨

#### 4. 타임아웃 문제 해결
- **문제**: 2021개 스트림 처리 시 5분 타임아웃 초과로 1년 전 fallback 발생
- **해결**: 타임아웃 10분으로 연장 + 워커 20개로 증가
- **결과**: 대용량 스트림도 안정적으로 처리 가능

#### 5. 첫 실행 초기화 최적화
- **전체 백업 모드**: 첫 실행 시 State 초기화만 수행 (로그 수집 건너뛰기)
- **안정성**: Lambda 타임아웃 위험 최소화
- **효율성**: 초기화와 백업을 분리하여 실패 지점 명확화

#### 6. 실제 성능 테스트 결과
```
테스트 환경: 10개 스트림, 5개 워커
- 순차 처리: 3.02초
- 멀티스레딩: 0.74초
- 성능 향상: 약 4배

대용량 환경 (2021개 스트림 실제 테스트):
- 이전: 5분 타임아웃 초과 → 1년 전 fallback
- 개선: 10분 타임아웃 + 20개 워커 → 정상 처리
- 결과: 43,288개 로그 수집 성공 (114초 소요)
...
### 압축 성능
- **압축률**: 95%+ (gzip 압축)
- **저장 공간**: 원본 대비 1/20 크기로 절약
- **예시**: 800KB 로그 → 22KB 압축 파일

## 🔧 대용량 스트림 처리 최적화

### 타임아웃 문제 해결

라이브 환경에서 2000+ 스트림이 있는 로그그룹 처리 시 발생하는 타임아웃 문제를 완전히 해결했습니다:

#### 문제 상황
```
로그: "324 (of 2021) futures unfinished"
- 총 스트림: 2021개
- 워커: 10개
- 타임아웃: 5분 (300초)
- 결과: 타임아웃 초과 → 1년 전 fallback → S3 파일 미생성
```

#### 해결 방안
1. **타임아웃 시간 증가**: 5분 → 10분
2. **워커 수 증가**: 10개 → 20개
3. **오류 메시지 상세화**: TimeoutError vs 일반 Exception 구분
4. **첫 실행 최적화**: State 초기화와 백업 분리

### 첫 실행 초기화 최적화

전체 백업 모드(`full_backup=true`)에서 첫 실행 시 안정성을 극대화하는 새로운 로직:

#### 동작 방식
```
첫 실행 (전체 백업 모드):
1. State 파일 없음 감지
2. 임시 State 파일 생성 (타임아웃 방지)
3. 2021개 스트림에서 가장 오래된 로그 검색 (10분 타임아웃)
4. State 파일에 실제 가장 오래된 시간 저장
5. 로그 수집 건너뛰기 (다음 실행부터 시작)

두 번째 실행부터:
1. State 파일 기반으로 30분씩 점진적 백업
2. 안정적이고 예측 가능한 처리
```
...
- **안정성**: Lambda 타임아웃 위험 최소화
- **효율성**: 초기화와 백업을 분리하여 실패 지점 명확화
- **예측성**: 점진적 백업으로 처리 시간 예측 가능
- **조건부 적용**: 전체 백업 모드에서만 적용 (일반 모드 영향 없음)
