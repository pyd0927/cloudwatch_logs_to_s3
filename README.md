...
- **복구 모드**: 실행 지연 시 자동으로 복구하여 데이터 손실 방지
...
### 동작 조건
- **실행 간격**: `MINUTES_BACK` 시간 이상 지연 시 복구 모드 활성화
- **복구 방식**: 현재 실행 시간 기준으로 Retention 삭제 시점 재계산

### 예시
- **MINUTES_BACK**: 30분
- **마지막 실행**: 1시간 전
- **복구 모드**: 현재 시간 기준으로 Retention 삭제 시점부터 30분 전까지 로그 수집
